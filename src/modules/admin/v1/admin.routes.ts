...
export class AdminRoute extends BaseRoute {
  ...
  private async init() {
     ...
     * Admin login router
     */
    this.router.post('/login', adminValidators.adminLogin, async (req, res) => {
      try {
    ...
    this.router.get('/profile', authorization.admin, async (req, res) => {
      try {
        let result = await adminController.profileData({
